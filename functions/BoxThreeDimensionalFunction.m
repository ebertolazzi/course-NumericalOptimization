...
classdef BoxThreeDimensionalFunction < FunctionMap
  ...
  methods
    ...
    function self = BoxThreeDimensionalFunction( M )
      ...
      % Those steps above are done here to save computations
    ...
    function f = evalMap(self,x)
      ...
      f    = exp( -t(i).*x(1) ) - exp( -t(i).*x(2) ) - x(3).*( exp( -t(i) ) - exp( -10.*t(i) ) ) ; 
    ...
    function J = jacobian( self, x )
      ...
      J  = [ -t(i).*exp( t(i).*x(1) ) , t(i).*exp( t(i).*x(2) ) , - ( exp( -t(i) ) - exp( -10.*t(i) ) ) ];
